...
class MultipleItemView: UIView {
    ...
    class func returnViewHeight(tagTexts: Array<Any>,
                                viewWidth: CGFloat,
                                eachNum: NSInteger,
                                hmargin: CGFloat,
                                vmargin: CGFloat,
                                tagHeight: CGFloat,
                                tagTextFont: UIFont) -> CGFloat{
    ...
    func setTagTexts(tagTexts:Array<Any>) {
        ...
        if self.eachNum > 0 {
            ...
            for i in 0...self.buttonTags.count-1 {
                ...
                if i < tagTexts.count {
                    let textStr = tagTexts[i] as! NSString
                    let itemWidth = MultipleItemView.returnTextWidth(text: textStr, font: tagTextFont!, viewWidth: viewWidth).width+20
                    totalWidth = totalWidth+CGFloat(itemWidth)+hmargin
                    if totalWidth - hmargin > viewWidth {
                        totalWidth = itemWidth+hmargin
                        row = row + 1
                        button.frame = CGRect(x: 0, y: CGFloat(row)*(self.tagHeight+self.vmargin), width: itemWidth, height: self.tagHeight)
                    } else {
                        button.frame = CGRect(x: totalWidth-itemWidth-self.hmargin, y: CGFloat(row)*(self.tagHeight+self.vmargin), width: itemWidth, height: self.tagHeight)
                    }
                    button.isHidden = false
                    button.setTitle((tagTexts[i] as! String), for: .normal)
                } else {
